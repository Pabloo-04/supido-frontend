...
interface Props {
  onCreated: (driver: DeliveryPerson) => void;
  onCancel: () => void;
}
...
export default function CreateDriverForm({ onCreated, onCancel }: Props) {
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      const driver = await createDeliveryPerson(selected.id);
      onCreated(driver);
    } catch (err) {
      ...
      {!selected ? (
        <>
          <p className="text-xs text-[var(--color-suido-4)]" style={{ fontFamily: "var(--font-dm)" }}>
            Paso 1 — Seleccioná un usuario existente para asignarle el perfil de repartidor.
          </p>
          ...
          {loadingUsers ? (
            ...
            <p className="text-center py-4 text-sm text-[var(--color-suido-3)]" style={{ fontFamily: "var(--font-dm)" }}>
              ...
              {filtered.map((u) => (
                <button
                  ...
                  className="flex items-center justify-between px-4 py-3
                             bg-[var(--color-suido-1)] hover:bg-[var(--color-suido-accent)]/10
                ...
                >
                  <div>
                    ...
                    <p className="text-[0.72rem] text-[var(--color-suido-4)]" style={{ fontFamily: "var(--font-dm)" }}>
                      {u.email}{u.phone ? ` · ${u.phone}` : ""}
                    </p>
                  </div>
                  <span className="text-[0.65rem] text-[var(--color-suido-accent)] font-semibold flex-shrink-0" style={{ fontFamily: "var(--font-dm)" }}>
                    Seleccionar →
                  </span>
                </button>
              ))}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3">
              <button
                ...
                disabled={page === 0}
                className="text-xs px-3 py-1.5 rounded-lg border border-[var(--color-suido-3)]/30
        ...
        <>
          <p className="text-xs text-[var(--color-suido-4)]" style={{ fontFamily: "var(--font-dm)" }}>
            Paso 2 — Confirmá la creación del perfil de repartidor.
          </p>
          ...
          <div className="bg-[var(--color-suido-1)] border border-[var(--color-suido-accent)]/25 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-[0.72rem] text-[var(--color-suido-4)]" style={{ fontFamily: "var(--font-dm)" }}>
                {selected.email}{selected.phone ? ` · ${selected.phone}` : ""} · ID #{selected.id}
              </p>
            </div>
            <button
              type="button"
              onClick={() => setSelected(null)}
              className="text-xs text-[var(--color-suido-4)] hover:text-white transition-colors flex-shrink-0"
              style={{ fontFamily: "var(--font-dm)" }}
            >
              Cambiar
            </button>
          ...
          <p className="text-xs text-[var(--color-suido-3)]" style={{ fontFamily: "var(--font-dm)" }}>
            El repartidor se creará como <span className="text-white font-medium">No disponible</span>. Puede activarse desde su propio panel.
          </p>
        </>
      )}
