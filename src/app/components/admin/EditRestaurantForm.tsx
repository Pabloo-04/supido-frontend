...
import { updateRestaurant, type Restaurant } from "@/lib/admin";
...
const CATEGORIES = [
    "FAST_FOOD", "ITALIAN", "MEXICAN", "CHINESE", "BREAKFAST",
    "DESSERTS", "HEALTHY", "PIZZA", "SEAFOOD", "VEGETARIAN",
];

export default function EditRestaurantForm({ restaurant, onUpdated, onCancel }: Props) {
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : "Error al actualizar restaurante.");
        } finally {
            setLoading(false);
        }
    }

    const textFields: { name: keyof typeof form; label: string; placeholder: string; required?: boolean }[] = [
