...
export default function EditUserForm({ user, onUpdated, onCancel }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!username.trim() || !email.trim()) {
      setError("Usuario y correo no pueden estar vacíos.");
      return;
    }
    ...
    setError(null);
    try {
      // If only the role changed use the dedicated endpoint; otherwise use the general patch
  ...
  return (
    <form
      ...
      className="bg-[var(--color-suido-2)] border border-[var(--color-suido-accent)]/30
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span
          className="text-[0.62rem] text-[var(--color-suido-4)] font-medium"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Field label="Usuario"      id="eu-username" value={username} onChange={setUsername} placeholder="username" />
        <Field label="Correo"       id="eu-email"    value={email}    onChange={setEmail}    placeholder="correo@ejemplo.com" />
        <Field label="Teléfono"     id="eu-phone"    value={phone}    onChange={setPhone}    placeholder="+34600000000" />
        <Field
