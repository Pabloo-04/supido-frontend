...
export async function fetchRestaurantOrders(
  ...
  const res = await fetch(
    `${BASE}/api/orders/restaurant/${restaurantId}?${qs}`,
    { headers: authHeaders() },
  );
  const json = await unwrap<
    RestaurantOrder[] | { content?: RestaurantOrder[]; totalPages?: number; totalElements?: number }
  >(res);
...
export async function fetchMyMenuItems(
  ...
  page = 0,
  size = 50,
): Promise<{ items: RestaurantMenuItem[]; totalPages: number; totalElements: number }> {
  const qs = new URLSearchParams({ page: String(page), size: String(size) });
  const res = await fetch(
    `${BASE}/api/restaurants/${restaurantId}/menu-items?${qs}`,
    { headers: authHeaders() },
  );
  const json = await unwrap<
    RestaurantMenuItem[] | { content?: RestaurantMenuItem[]; totalPages?: number; totalElements?: number }
  >(res);
  ...
  return {
    items: Array.isArray(paged.content) ? paged.content : [],
    totalPages: paged.totalPages ?? 1,
    totalElements: paged.totalElements ?? 0,
  };
}
...
export async function toggleMenuItemAvailability(
  ...
  const res = await fetch(
    `${BASE}/api/restaurants/${restaurantId}/menu-items/${itemId}/availability`,
...
export async function assignDeliveryPerson(
  ...
  const res = await fetch(
    `${BASE}/api/orders/${orderId}/assign-delivery-person?deliveryPersonId=${deliveryPersonId}`,
    { method: "PATCH", headers: authHeaders() },
  );
  if (!res.ok) {
    const err = await res.json().catch(() => null);
...
export async function fetchCategories(): Promise<string[]> {
  const res = await fetch(`${BASE}/api/restaurants/categories`, {
    headers: authHeaders(),
  });
...
export async function fetchMyRestaurant(): Promise<MyRestaurant | null> {
  const res = await fetch(`${BASE}/api/restaurants/my-restaurant`, {
    headers: authHeaders(),
  });
